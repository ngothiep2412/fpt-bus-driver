...
class UpdateProfileController extends GetxController {
  ...
  UserModel user = UserModel.fromJson(GetStorage().read('driverUser') ?? {});
  var informationUser =
      UserModel.fromJson(GetStorage().read('driverUser') ?? {}).obs;

  TextEditingController nameController = TextEditingController();
  TextEditingController phoneController = TextEditingController();

  ProfileController profileController = Get.find();
  MyDrawerHeaderController myDrawerHeaderController = Get.find();
        ...
        if (accessToken != null) {
          if (imageFile == null) {
            ResponseApi responseApi = await usersProviders
                .updateProfileWithoutPicture(name, phone, accessToken, user);
            final dataUser = responseApi.data;
            if (dataUser != null) {
              profileController.user.value =
                  UserModel.fromJson(responseApi.data);

              myDrawerHeaderController.user.value =
                  UserModel.fromJson(responseApi.data);
              if (GetStorage().hasData('driverUser')) {
                GetStorage().remove('driverUser');
              }
              GetStorage().write('driverUser', dataUser);
            ...
            // String? jwtToken = await storage.read(key: 'jwtToken');
            ResponseApi responseApifirst = await usersProviders.uploadPicture(
                user, base64Image, accessToken);
